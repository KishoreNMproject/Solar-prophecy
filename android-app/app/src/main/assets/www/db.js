...
export async function importBackup(db, backup) {
  ...
  if (db.objectStoreNames.contains(TOMBSTONES_STORE)) {
    ...
    tombStore.clear(); // Queue clear, no await
    if (backup.tombstones && Array.isArray(backup.tombstones)) {
      for (const t of backup.tombstones) {
        tombStore.put(t); // Queue put, no await
      }
    }
    // Wait for all tombstone operations to be committed
    await new Promise((resolve, reject) => {
      tombTx.oncomplete = resolve;
      tombTx.onerror = () => reject(tombTx.error);
    });
  }
